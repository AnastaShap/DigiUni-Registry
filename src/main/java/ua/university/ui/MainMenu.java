...
import ua.university.TCP.TcpRegistryServer;
...
public class MainMenu {
    ...
    private TcpRegistryServer tcpServer;
    ...
    public void run() {
        ...
        while (true) {
            printMenu();
            int option = ConsoleInputValidator.readMenuOption(scanner, 0, 20);
    ...
    private void startTcpMode() {
        if (tcpThread != null && tcpThread.isAlive()) {
            System.out.println("TCP server is already running.");
            return;
        }

        tcpServer = new TcpRegistryServer(9090, studentService, logger);
        tcpThread = new Thread(tcpServer::start, "tcp-registry-server");
    ...
    private void stopTcpMode() {
        ...
        try {
            ...
            System.out.println("TCP stop error: " + e.getMessage());
        }
    ...
    private void printMenu() {
        ...
        System.out.println("19 - Start TCP server (admin)");
        System.out.println("20 - Stop TCP server (admin)");
