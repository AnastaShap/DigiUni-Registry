...
class AuthStorageAndDomainTest {
    ...
    void dataStorageServiceSavesAndLoadsSnapshot() throws Exception {
        ...
        // Студент (конструктор приймає String для email/phone, які всередині стають DTO)
        Student student = new Student(
                "500", "Ткаченко", "Марія", "Іванівна",
                LocalDate.of(2004, 1, 10),
                "maria@ukma.edu.ua", "+380501111111",
                "S500", 2, "ІПЗ-2", 2022,
                StudyForm.BUDGET, StudentStatus.STUDYING
        );

        // ВИПРАВЛЕНО: Створюємо викладача, щоб передати його в snapshot
        Teacher teacher = new Teacher(
                "T-1", "Іваненко", "Олег", "Петрович",
                LocalDate.of(1985, 5, 5),
                new Email("teacher@ukma.edu.ua"), new PhoneNumber("+380501234567"),
                "Professor", "PhD", "Docent", LocalDate.of(2015, 9, 1), 1.0
        );

        // ВИПРАВЛЕНО: Передаємо створеного викладача в список
        ...
        assertEquals(1, loaded.teachers().size()); // Перевірка завантажених вчителів
